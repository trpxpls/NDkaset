// all_products_script.js (สำหรับ all_products.html)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // เรนเดอร์สินค้าทั้งหมดเมื่อหน้านี้โหลด
    renderFilteredProducts('all'); // เรียกใช้ฟังก์ชันเดิมแต่ส่ง 'all' ไป
    ...
    // Event Delegation สำหรับปุ่มในรายการสินค้า (ลบ, แก้ไข, ส่งออก, เข้า) ในหน้ารายงาน
    productList.addEventListener('click', (event) => {
        ...
        if (target.classList.contains('delete-btn')) {
            if (confirm(`คุณต้องการลบ "${products[productIndex].name} (${products[productIndex].quantity} ${products[productIndex].unit})" ใช่หรือไม่?`)) {
                products.splice(productIndex, 1);
                saveProducts();
                renderFilteredProducts('all'); // เรนเดอร์ใหม่
            }
            ...
            sessionStorage.setItem('editProductTimestamp', productToEdit.timestamp); // ส่ง timestamp ไปยัง sessionStorage
