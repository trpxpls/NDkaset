// incoming_products_script.js (สำหรับ incoming_products.html)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // เรนเดอร์สินค้าเข้าเมื่อหน้านี้โหลด
    renderFilteredProducts('incoming');
    ...
    productList.addEventListener('click', (event) => {
        ...
        if (target.classList.contains('dispatch-btn')) {
            ...
            if (confirm(`คุณต้องการทำเครื่องหมาย "${productToDispatch.name}" เป็นสินค้าส่งออกหรือไม่?`)) {
                ...
                saveProducts();
                renderFilteredProducts('incoming'); // เรนเดอร์ใหม่
            }
    ...
    function renderFilteredProducts(filterType) {
        ...
        if (filterType === 'incoming') {
            ...
            filteredProducts.sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp)); // เรียงใหม่สุดไปเก่าสุด
        } else if (filterType === 'outgoing') { // ไม่น่าจะถูกเรียกใช้ในหน้านี้
            filteredProducts = products.filter(product => product.outgoingTimestamp);
